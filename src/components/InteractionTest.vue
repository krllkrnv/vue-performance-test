...
async function measureAction(action) {
  action.execute()
  await nextFrame()  // ждем минимум 2 кадра
  await sleep(400)   // небольшая пауза для визуализации и layout shift
  if (action.reset) action.reset()
  await nextFrame()  // ждем рендер после сброса
  await sleep(400)   // пауза после сброса
}
...
async function runTest() {
  ...
  try {
    ...
    for (let i = 0; i < props.cycles; i++) {
      ...
      const actions = [
        { execute: () => { filterText.value = data.value[idx].name; applyFilter() }, reset: () => { appliedFilter.value = '' } },
        { execute: () => sortBy(['id', 'name', 'value'][i % 3]), reset: () => { sortKey.value = '' } },
        { execute: () => toggleDetails(data.value[idx].id), reset: () => { expandedRow.value = null } }
...
const displayData = computed(() => {
  let arr = data.value
  if (appliedFilter.value) arr = arr.filter(item => item.name.includes(appliedFilter.value))
  if (sortKey.value) arr = [...arr].sort((a, b) => {
...
.info { font-weight: bold; margin-bottom: 15px; padding-bottom: 10px; border-bottom: 1px solid #ddd; position: relative; }
.status { font-weight: normal; font-size: 0.9em; color: #666; margin-top: 5px; }
.controls { margin-bottom: 15px; display: flex; gap: 10px; }
.controls input { flex: 1; padding: 8px 12px; border: 1px solid #ddd; border-radius: 4px; }
.controls button { padding: 8px 16px; background-color: #4a7cff; color: white; border: none; border-radius: 4px; cursor: pointer; transition: background-color 0.2s; }
.controls button:hover { background-color: #3a6ae8; }
...
.details { padding: 12px; background-color: #eef; border-radius: 4px; margin: 5px 0; }
